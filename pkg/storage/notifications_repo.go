...
func (db *DB) InsertNotification(notifType string, actor, targetCID, relatedCID []byte, timestamp int64) error {
	_, err := db.Exec(
...
func (db *DB) GetNotifications(before int64, limit int) ([]NotificationRow, error) {
	if before == 0 {
		before = time.Now().UnixMilli() + 1
	}
	rows, err := db.Query(
		`SELECT id, type, actor, target_cid, related_cid, timestamp, read
		 FROM notifications
		 WHERE timestamp < ?
	...
	for rows.Next() {
		...
		var readInt int
		if err := rows.Scan(&n.ID, &n.Type, &n.Actor, &n.TargetCID, &n.RelatedCID, &n.Timestamp, &readInt); err != nil {
...
func (db *DB) UnreadCount() (int, error) {
	var count int
