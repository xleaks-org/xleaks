...
function truncateActor(actor: string): string {
...
function getNotificationText(type: string, actor: string): string {
  const name = truncateActor(actor);
  switch (type) {
    case 'like':
...
function getNotificationHref(notification: Notification): string {
  switch (notification.type) {
    ...
    case 'dm':
      return `/messages/${notification.actor}`;
    default:
      return notification.targetCid ? `/post/${notification.targetCid}` : '#';
  }
}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm text-white">
          {getNotificationText(notification.type, notification.actor)}
        </p>
        {notification.targetCid && notification.type !== 'follow' && (
